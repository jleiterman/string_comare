...
def evaluateinputstring(inputstring,shortstring,fullset,revealedset):
    ...
    if blocks[0][2] == 0:
        ...
        matchedstring = shortstring[blocks[0][1]:blocks[-2][1]+blocks[-2][2]] #fails if input string is not in the short string at all?
        s2= difflib.SequenceMatcher(None, inputstring, matchedstring, autojunk=False)
        matchratio = s2.ratio()
    print("match Ratio:" +str(matchratio))
    if matchratio > .9:
...
while continue_var:
    ...
    fullset = set(range(0,len(shortstring)))
    revealedset = set()
    for i in fullset:
